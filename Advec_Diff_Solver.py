...
def Advec_Diff_Adjoint_Solver_Steady_State(scheme,rho,V,Gamma_phi,desired_Residual,Element_Element_Connectivity_new,Source_Term_diff,Source_Term_advec,Element_cen,Element_Edge_Connectivity_new,Edge_Len,Diffusion_mesh_data,Boundary_Edges,Phi_0):
    """
    Input:
    scheme: 0: CDS, 1: Upwind
    rho: Density
    ...
    Element_grad_phi_LS = Grad_Phi_LS(Element_Element_Connectivity_new,Element_cen,Phi_0)
    mod_grad_phi = (np.sum(Element_grad_phi_LS[:,1:]**2,axis=1))**0.5
    ...
    while res > desired_Residual:
        ...
        for i in range(Element_Element_Connectivity_new.shape[0]):
            ...
            for j in range(Nb_Elements.shape[0]):
                ...
                if common_edge.shape[0] == 1:
                    ...
                    V_ip = V
                    n_ip = neg_k_p_prime_data(Diffusion_mesh_data,Element,Edge)
                    # This -ve sign because of the adjoint
                    m_dot_ip = -(rho*(np.dot(V_ip,n_ip))*ds)
        ...
        # print(f"Condition number: {np.linalg.cond(A_mat)}")
        ...
        phi = ml.solve(RHS,accel="gmres", x0=phi_0,tol=desired_Residual)
        # phi = ml.solve(RHS,accel="cg", x0=phi_0,tol=desired_Residual)
...
def Advec_Diff_Solver_Steady_State_A_b(scheme,rho,V,Gamma_phi,desired_Residual,Element_Element_Connectivity_new,Source_Term_diff,Source_Term_advec,Element_cen,Element_Edge_Connectivity_new,Edge_Len,Diffusion_mesh_data,Boundary_Edges):
    """
    Input:
    scheme: 0: CDS, 1: Upwind
    rho: Density
